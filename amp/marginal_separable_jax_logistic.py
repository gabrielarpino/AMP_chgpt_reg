...
def generate_uniform_random(n):
    return jnp.array(nprandom.uniform(size=(n, 1)))

def q_j_sample(Θ, c_j,L, ϵ=None):
    """Sample output yj according to signal index cj. θ=XB.
    This is used to sample synthetic data.
    
    We take as input ϵ that was sampled according to the uniform distribution using for example generate_uniform_random() (above).
    This is because jax does cannot perform random sampling (it uses the same key for all samples)."""
    ...
    # only sample if the random component is not supplied as one of the inputs
    if ϵ is None:
        ϵ = np.random.uniform(size=num_samples)
    
    P = 1/(1+jnp.exp(-jnp.multiply(Θ, (Ψ == 1).astype(int)).sum(axis=1)))
    Y = jnp.greater(P, ε).astype(int).reshape((num_samples, 1)) 
    return Y

def q_j_sample_int(Θ, c_j,L, ϵ=None):
    """Sample output yj according to signal index cj. θ=XB.
    We use an approximation with tanh function to make the function integrable.
    This is used by the sampling procedured inside the AMP algorithm. """
    if Θ.ndim == 1:
        Θ = Θ.reshape((1, L))
    num_samples = Θ.shape[0]
    Ψ = jnp.zeros((num_samples, L))
    ...
    P = 1/(1+jnp.exp(-jnp.multiply(Θ, (Ψ == 1).astype(int)).sum(axis=1)))
    Y = ((1+jnp.tanh(50*(P - ε)))/2).reshape((num_samples, 1)) 

    return Y

def q(Θ, C, ϵ):
    """ This takes in Θ ∈ R^{nxL} and C ∈ {0,L-1}^{nxL} and returns Y ∈ R^{nx1}. The function also takes as input 
    a vector of ranom elements ϵ to account for the fact that jax uses the same random key for all samples drawn 
    inside the vmap."""

    L = Θ.shape[1]
    return jax.vmap(q_j_sample, in_axes=(0, 0, None, 0), \
                    out_axes=0)(Θ, C, L, ϵ).reshape((Θ.shape[0], 1))
...
def form_log_numerator_0(k, V, ρ_s, ϕ_s, μ_s, Σ_Z):
    """ Obtain the numerator of the expectation E(Z_k|V,u,ϕ_s) for a given signal configuration ϕ_s for the case when the observation is 0."""
    cov_k_s = Σ_Z[k,:]
    ϕ_s = ϕ_s.flatten()
    μ_k = μ_s[k]

    res = np.sum(((μ_k)  - μ_k*norm.cdf(ρ_s * μ_s, loc=0, scale=1) - (ρ_s*cov_k_s)* norm.pdf(ρ_s * μ_s, loc=0, scale=1))*ϕ_s)
    res_sign = 2*(res>0)-1
    res = jnp.log(jnp.abs(res))

    return res, res_sign


def form_log_numerator_1(k, V, ρ_s, ϕ_s, μ_s, Σ_Z):
    """ Obtain the numerator of the expectation E(Z_k|V,u,ϕ_s) for a given signal configuration ϕ_s for the case when the observation is 1."""
    ϕ_s = ϕ_s.flatten()
    μ_k = μ_s[k]
    cov_k_s = Σ_Z[k,:]

    res = np.sum((μ_k* norm.cdf(ρ_s*μ_s,loc=0, scale=1) + (ρ_s*cov_k_s)*norm.pdf(ρ_s*μ_s))* ϕ_s)
    res_sign = 2*(res>0)-1
    res = jnp.log(jnp.abs(res))
...
form_log_num_1_all =  jax.vmap(form_log_numerator_1, in_axes=(0,None,None,None,None,None), out_axes=(0,0))
form_log_num_0_all =  jax.vmap(form_log_numerator_0, in_axes=(0,None,None,None,None,None), out_axes=(0,0))
...
def E_Z(V, u, ϕ_j, σ, ρ, ν, κ_T):
    ...
    ϕ_s = ϕ_j.reshape((L, 1)).flatten()
    Σ = ν.T @ jnp.linalg.pinv(ρ) @ ν + κ_T     # covariance matrix of V
    Σ_Z_given_V = (ρ - ν.T @ jnp.linalg.pinv(Σ) @ ν)
    μ_s = ν @ jnp.linalg.pinv(Σ) @ (V.reshape(L,1).flatten())

    σ_sq_s = Σ_Z_given_V.diagonal().flatten()

    ρ_s = lam/jnp.sqrt(1+lam**2*σ_sq_s)

    log_num_arr_0, sign_num_arr_0 = form_log_num_0_all(jnp.arange(L),V, ρ_s, ϕ_s, μ_s, Σ_Z_given_V)
    log_cdf = norm.logcdf(-ρ_s*μ_s, loc=0, scale=1)
    log_denom_0 = logsumexp(a=log_cdf, b=ϕ_s)

    log_num_arr_1, sign_num_arr_1 = form_log_num_1_all(jnp.arange(L),V, ρ_s, ϕ_s, μ_s, Σ_Z_given_V)
    log_cdf = norm.logcdf(ρ_s*μ_s, loc=0, scale=1)
    log_denom_1 = logsumexp(a=log_cdf, b=ϕ_s)


    res_0 = (sign_num_arr_0  * jnp.exp(log_num_arr_0 - log_denom_0)).reshape((1, L)) 
    res_1 = (sign_num_arr_1  * jnp.exp(log_num_arr_1 - log_denom_1)).reshape((1, L))

    # Functions ind_1 and ind_0 are approximations of indicator functions required to make function g differentiable. 
    # The scaling factor 50 used below can be adjusted to change the 'steepness' of the sigmoid function at the risk of approaching the step function.
    # The greater the scaling factor, the closer the approximations are to step function.
    ind_1 = lambda x: 1/(1+jnp.exp(-50*(x-0.5)))
    ind_0 = lambda x: 1/(1+jnp.exp(50*(x-0.5)))
    res =  ind_0(u)*res_0 + ind_1(u)*res_1
...
def g_j(V, u, ϕ_j, σ, ρ, ν, κ_T):
    ...
    ξ = jnp.linalg.pinv(ν.T @ jnp.linalg.inv(ρ) @ ν + κ_T + 1e-16*jnp.eye(ρ.shape[0]))
    p_inv_rhs = ρ - ν.T @ ξ.T @ ν + 1e-16*jnp.eye(ρ.shape[0])
    E_Z_ = E_Z(V, u, ϕ_j, σ, ρ, ν, κ_T) 
    # tmp tends to have very large condition number, so we use pinv instead of solve:
    # tmp = ν.T @ jax.scipy.linalg.inv(ρ) @ ν + κ_T
    return (E_Z_ - V @ ξ.T @ ν.T) @ jnp.linalg.pinv(p_inv_rhs)
...
#REMOVED JIT
g = jax.vmap(g_j, in_axes=(0, 0, 1, None, None, None, None), out_axes=0)
...
def g_Z_j(V, Z, c_j, ϕ_j, ϵ, ρ, ν, κ_T):
    L = ρ.shape[0]
    # jax.debug.print("q().shape: {x}", x = q_j_sample(Z, c_j, σ, L).shape)
    # jax.debug.print("q(): {x}", x = q_j_sample(Z, c_j, σ, L))
    return g_j(V, q_j_sample_int(Z, c_j, L, ϵ)[0], ϕ_j, 0, ρ, ν, κ_T)


def dgdZ_j(V_j, Z_j, c_j, ϕ_j, ϵ_j, ρ, ν, κ_T):
    L = ρ.shape[0]
    return jax.jacfwd(g_Z_j, 1)(V_j, Z_j, c_j, ϕ_j, ϵ_j, ρ, ν, κ_T).reshape((L, L)) 
# dgdZ_j_jit = jax.jit(dgdZ_j, static_argnames=("σ")) # LxL
dgdZ_j_jit = jax.jit(dgdZ_j) # LxL
# REMOVED JIT
dgdZ = jax.vmap(dgdZ_j, in_axes=(0, 0, 0, 1, 0, None, None, None), out_axes=0)#, static_argnames=("σ")) # nxLxL
...
def κ_B_marginal(ϕ, ρ, n, σ, ν, κ_T, ρ_est = None, num_samples = 1000):
    ...
    def E(ϕ_and_j_col_j): 
        ...
        return jnp.sum(jax.vmap(sm, 0, 0)(jnp.arange(L)), axis = 0) # sum over c_j, small vmap over L
        # sum over all L signal indices c_j for the j-th observation. dim LxL

    # orig_res = 1/n * jnp.sum(jax.vmap(E, 1, 0)(ϕ_and_j), axis=0) # average over all n g_j's. dim LxL
    # return orig_res
    # Replace the above with a map rather than a vmap, for memory efficiency: 

    new_res = 1/n * jnp.sum(jax.lax.map(E, ϕ_and_j.T), axis=0) # more memory efficient
    return new_res
...
def ν̂_marginal(ϕ, ρ, n, σ, ν, κ_T, ρ_est = None, num_samples=1000):
    ...
    # orig_res = 1/n * jnp.sum(jax.vmap(E, 1, 0)(ϕ_and_j), axis=0) # average over all n g_j's. dim LxL    
    new_res = 1/n * jnp.sum(jax.lax.map(E, ϕ_and_j.T), axis=0) # more memory efficient, still to be checked on whether they are the same. 
    return new_res
...
def GAMP_gaussian_clean(B̂_0, δ, p, ϕ_, L, σ, X, Y, ρ, T, prior = None, verbose=False, seed=None, tqdm_disable = False):
    ...
    for t in tqdm(range(T), disable = tqdm_disable):
        ...
        ν̂ = κ_B_marginal(ϕ, ρ, n, σ, ν, κ_T, num_samples = 2000)
...
def GAMP_full(B̃, δ, p, ϕ_, σ, X, Y, C_true, T = 10, B̂_0 = None, 
    ...
    for t in tqdm(range(T), disable = tqdm_disable):
        ...
        try:
            ...
            print('=== EARLY TERMINATION: Floating Point')
        ...
        κ_B = κ_B_marginal(ϕ, ρ, n, σ, ν, κ_T, ρ_est, num_samples=1000)
        # print(f'κ_B avg = {κ_B}')
        ν̂ = κ_B
        ν̂_arr[t] = ν̂
        κ_B_arr[t] = κ_B
        ...
        ϵ_fixed = jnp.array(nprandom.uniform(size=(n, num_samples)))
        ...
        def sm(ϕ_j, j, c_j):
            ...
            u = q_j_sample(Z_B_fixed[j, :, :], c_j, L) # TODO: THIS MAY HAVE TO BE CHANGED! SAMPLING q_j AGAIN,  likely uses the same sample in each, likely not such a huge problem since it samples a whole ϵ of size num_samples, empirical match indicates that two different \kappa_B are not too different (1e-7). 
            # g_j_ = g_j(V[j, :].flatten(), q_j(Z_B[j, :].reshape((1, L)), c_j, σ).sample(), ϕ_j, σ, ρ, ν, κ_T).reshape((1, L))
            # NOTE: input V, u to g are sampled according to the true signal prior, but g itself
            # is defined using the estimated signal prior ρ_est.
            g_j_full = jax.vmap(g_j, in_axes=(0, 0, None, None, None, None, None), 
        ...
        # κ_B_fixed_elements = jax.vmap(sm, in_axes=0, out_axes=0)(ϕ.T, jnp.arange(n), C_true) # Nested vmap is not good for memory. 
        # Replace the above with jax.map for improved memory usage:. Complete. 
        def κ_B_fixed_elements_fn(ϕ_T, range_arr, C): 
            def helper(x): 
                ϕ_j = x[:L]
                j = x[L].astype(int)
                c_j = x[L+1].astype(int)
                return sm(ϕ_j, j, c_j)
            x = jnp.column_stack((ϕ_T, range_arr, C_true))
            return jax.lax.map(helper, x)
        κ_B_fixed_elements = κ_B_fixed_elements_fn(ϕ.T, jnp.arange(n), C_true)
        ...
        def estimate_ν̂(V, Z, ϵ):
            dgdZ_ = dgdZ(V, Z, C_true, ϕ, ϵ, ρ_est, ν, κ_T)
            return 1/n * jnp.sum(dgdZ_, axis=0)
        # ν̂_fixed = jnp.mean(jax.vmap(estimate_ν̂, in_axes=(1, 1, 1), out_axes=0)(V_θ_fixed, Z_B_fixed, ϵ_fixed), axis=0) 
        # Replaced the above with jax.map for improved memory usage. Tested: they match.
        def ν̂_fixed_fn(range_arr):

            def helper(j):
                v = V_θ_fixed[:, j, :]
                z = Z_B_fixed[:, j, :]
                e = ϵ_fixed[:, j]
                return estimate_ν̂(v, z, e)
            return jax.lax.map(helper, range_arr)
        ν̂_fixed = jnp.mean(ν̂_fixed_fn(jnp.arange(num_samples)), axis=0)
    ...
    if early_termination_flag:      
        for i in range(t-1,T):
            ...
            # ν̂_arr[t] = ν̂
            # κ_B_arr[t] = κ_B
            ν_fixed_arr[i+1] = ν_fixed_arr[t-1]
            κ_T_fixed_arr[i+1] = κ_T_fixed_arr[t-1]
            ν_arr[i+1] = ν_arr[t-1]
            κ_T_arr[i+1] = κ_T_arr[t-1] 
            κ_B_fixed_arr[i] = κ_B_fixed_arr[t-2]
            ν̂_fixed_arr[i] = ν̂_fixed_arr[t-2]
            ν̂_arr[i] = ν̂_arr[t-2]
            κ_B_arr[i] = κ_B_arr[t-2]
...
def ν_κ_T_mc(δ, p, κ_B, ν̂, true_signal_prior: SignalPrior, \
             est_signal_prior = None, st_ζ= None, num_samples=5000):
    """
...
def SE_fixed_C_v1(C_true, true_signal_prior: SignalPrior, \
    ...
    ρ_est = jnp.array(1/δ * est_signal_prior.cov)
    ν_fixed = jnp.zeros((L, L)) 
    ν_fixed_arr = jnp.zeros((T+1, L, L))
    # ν_fixed_arr[0] = ν_fixed
    ν_fixed_arr = ν_fixed_arr.at[0].set(ν_fixed)

    κ_T_fixed = ρ_est 
    κ_T_fixed_arr = jnp.zeros((T+1, L, L))
    # κ_T_fixed_arr[0] = κ_T_fixed
    κ_T_fixed_arr = κ_T_fixed_arr.at[0].set(κ_T_fixed)
    sq_corr_fixed_C = jnp.zeros((L, T))
    num_samples = 1000
    ...
    for t in (range(T)):
        ...
        Z_B = jax.random.multivariate_normal(subkeys[0], jnp.zeros(L), ρ, shape=(n, num_samples))
        V_Θ = Z_B @ jnp.linalg.inv(ρ) @ ν_fixed + jax.random.multivariate_normal(subkeys[1], jnp.zeros(L), κ_T_fixed, shape=(n, num_samples))
        assert Z_B.shape == (n, num_samples, L)
        assert V_Θ.shape == (n, num_samples, L)

        def estimate_ν̂(V, Z, ϵ):
            dgdZ_ = dgdZ(V, Z, C_true, ϕ, ϵ, ρ_est, ν_avg, κ_T_avg)
            ν̂_fixed_est = jnp.mean(dgdZ_, axis=0) # sum over n gives LxL matrix,
            assert ν̂_fixed_est.shape == (L, L)
            return ν̂_fixed_est
        ε = jax.random.uniform(subkeys[2], minval=0.0, maxval=1.0, shape=(n, num_samples))
        # TODO: Replace vmaps such as this one with jax scan for better memory efficiency.
        ν̂_fixed = jnp.mean(jax.vmap(estimate_ν̂, in_axes=(1, 1, 1), out_axes=0)(V_Θ, Z_B, ε), axis=0)
        ...
        V_B_i_samples = B_i_samples @ ν̂_fixed + jax.random.multivariate_normal(subkeys[2], jnp.zeros(L), κ_B_fixed, shape=(num_samples * p, ))
        ...
        ν_fixed = 1/δ * 1/(num_samples*p) * B_i_samples.T @ f_V
        # ν_fixed_arr[t+1] = ν_fixed
        ν_fixed_arr = ν_fixed_arr.at[t+1].set(ν_fixed)
        # only f itself uses the estimated signal prior, all other terms use the true signal prior
        diff = f_V - B_i_samples @ jnp.linalg.inv(ρ) @ ν_fixed 
        κ_T_fixed = 1/δ * (1/(num_samples*p)) * diff.T @ diff
        # κ_T_fixed_arr[t+1] = κ_T_fixed
        κ_T_fixed_arr = κ_T_fixed_arr.at[t+1].set(κ_T_fixed)
