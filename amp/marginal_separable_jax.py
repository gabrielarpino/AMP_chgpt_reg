...
def q_j_sample(Θ, c_j, η_j, L):
    ...
    Y = jnp.multiply(Θ, (Ψ == 1).astype(int)).sum(axis=1).reshape(
        (num_samples, 1)) + η_j
    return Y
...
def q(Θ, C, σ):
    ...
    L = Θ.shape[1]
    η = jnp.array(nprandom.normal(loc=0, scale=σ, size=(Θ.shape[0], 1))) # noise
...
def E_Z(V, u, ϕ_j, σ, ρ, ν, κ_T):
    ...
    return res
E_Z_mapped = jax.vmap(E_Z, in_axes=(0, 0, 1, None, None, None, None), out_axes=0)
...
def SE_fixed_C_v1(C_true, true_signal_prior: SignalPrior, \
    ...
    Adapted so it can be used for JIT compilation using JAX. 
    """
    if st_ζ is not None:
        assert type(est_signal_prior) == SparseGaussianSignal, \
    ...
    for t in (range(T)):
        ...
        # Actual inputs into g^*, V_θ, Z_B should follow the true signal prior. 
        # It is only g^* itself thats designed according to the estimated signal prior + random C.
        # Z_B = jnp.array(nprandom.multivariate_normal(np.zeros(L), np.array(ρ), size=(n, num_samples)))
        # V_Θ = Z_B @ jnp.linalg.inv(ρ) @ ν_fixed + \
        #     jnp.array(nprandom.multivariate_normal(jnp.zeros(L), κ_T_fixed, size=(n, num_samples)))

        # Sample using jax instead, I think we should be splitting this key every iteration at least. 
        Z_B = jax.random.multivariate_normal(key, jnp.zeros(L), ρ, shape=(n, num_samples))
        V_Θ = Z_B @ jnp.linalg.inv(ρ) @ ν_fixed + jax.random.multivariate_normal(key, jnp.zeros(L), κ_T_fixed, shape=(n, num_samples))
        ...
        # Sample with JAX instead
        V_B_i_samples = B_i_samples @ ν̂_fixed + jax.random.multivariate_normal(key, jnp.zeros(L), κ_B_fixed, shape=(num_samples * p, ))
        # Feed samples into f designed using the estimated signal prior
        if type(est_signal_prior) == GaussianSignal:
            f_V = f(V_B_i_samples, δ, ρ_est, ν̂_avg, κ_B_avg).reshape((num_samples*p, L))
        elif type(est_signal_prior) == SparseGaussianSignal:
        ...
        # for l in range(L):
        #     sq_corr_fixed_C[l, t] = ν_fixed[l, l]**2 / \
        #         ( ρ[l, l] * (κ_T_fixed + ν_fixed.T @ jnp.linalg.inv(ρ) @ ν_fixed)[l, l])
    if False:
        plt.figure()
        for l in range(L):
            plt.plot(sq_corr_fixed_C[l, :], label=f"β{l}")
