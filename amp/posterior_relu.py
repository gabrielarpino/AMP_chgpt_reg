...
import amp.marginal_separable_jax_relu
...
def tabulate_state_evolution(η_arr, true_signal_prior, est_signal_prior, \
    ...
    def SE_fixed_C_clean(ψ):
        return amp.marginal_separable_jax_relu.SE_fixed_C_v1(ψ, true_signal_prior, est_signal_prior, \
                                    δ, p, ϕ_, L, σ, T, \
...
### --- Functions from the approx posterior --- ###
...
def logP_Vj_Yj_given_ψj(Vj, Yj, ψj, ρ, σ, ν, κ_T):
    ...
    # see if adding P(V) changes anything

    μ_j = Vj.flatten()[ψj]
    σ2_j = (ρ-ν).diagonal()[ψj]
    ν_ρ_inv = ν.T @ jnp.linalg.inv(ρ)
    Σ = ν_ρ_inv @ ν + κ_T
    σ2_j = (ρ - ν.T @ jnp.linalg.inv(Σ) @ ν)[ψj, ψj]
    σ2 = σ**2
    μ_star_j = (Yj*σ2_j+μ_j*σ2)/(σ2+σ2_j)
    σ2_star_j = σ2*σ2_j/(σ2+σ2_j)

    ν_ρ_inv = ν.T @ jnp.linalg.inv(ρ)
    Σ = ν_ρ_inv @ ν + κ_T
    θ_0  = norm.logcdf(0, loc=μ_j, scale=jnp.sqrt(σ2_j)) + norm.logpdf(Yj, loc=0, scale=σ)

    θ_1 = norm.logcdf(0, loc=-μ_star_j, scale=jnp.sqrt(σ2_star_j))+norm.logpdf(Yj, loc=μ_j,scale= jnp.sqrt(σ2+σ2_j))
    
    res = logsumexp(a = jnp.array([θ_0, θ_1]))
    return  res # positive scalar
