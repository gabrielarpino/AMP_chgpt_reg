...
def q_j_sample(Θ, c_j, η, L):
    ...
    Y = jnp.maximum(jnp.multiply(Θ, (Ψ == 1).astype(int)).sum(axis=1).reshape(
        (num_samples, 1)), 0).flatten() + η
    return Y.reshape((num_samples, 1))

# Not used within this file
def q(Θ, C, η):
    """ This takes in Θ ∈ R^{nxL} and C ∈ {0,L-1}^{nxL} and returns Y ∈ R^{nx1}.
    Needs to be differentiable to go into SE_fixed_C. 
    We take as input η to account for the inability of jax to perform random sampling inside the vmap.
    jax applies the same seed to all samples drawn 'inside' the vmap - i.e. the noise applied to all samples
...
def form_log_theta_0_all(u, σ2, μ_l, σ2_l):
    """ 
    Calculate the log of θ_0 = N(u;0,σ2)Φ(0,μ_l,σ2_l) for all l∈(1,L)
    Input:
        u and σ2 are scalars. σ2 is the variance of the noise
        μ_l and σ2_l are Lx1

    The output is Lx1
    """
    #assert μ_l.shape[1] == 1
    #assert σ2_l.shape[1] == 1
    # as the sigmas get really small, the pdf goes to zero
    return norm.logcdf(0, loc=μ_l, scale=jnp.sqrt(σ2_l)) + norm.logpdf(u, loc=0, scale=jnp.sqrt(σ2))

def form_log_theta_1_all(u, σ2, μ_l, σ2_l):
    """ 
    Calculate the log of θ_0 = 1-Φ(0,μ_l*,σ2_l*) for all l∈(1,L)
    Input:
        u and σ2 are scalars. σ2 is the variance of the noise
        μ_l and σ2_l are Lx1
    ...
    μ_star_l = (u*σ2_l+μ_l*σ2)/(σ2+σ2_l)
    σ2_star_l = (σ2*σ2_l)/(σ2+σ2_l)

    # ERRORS HERE
    #cdfs = norm.cdf(0, loc=μ_star_l, scale=jnp.sqrt(σ2_star_l))
    #log_1 = jnp.zeros(L)
    #cdfs_log_1 = jnp.vstack((cdfs,log_1)).T
    #logsumexp(a=cdfs_log_1, axis=1, b = jnp.array([-1,1]))

    # sigmas get really small which makes the last term 'explode'
    #return jnp.log(1-cdfs)-0.5*(jnp.log(2*np.pi)+jnp.log(σ2+σ2_l))-0.5*jnp.power(u+μ_l, 2)/(σ2+σ2_l)
    return norm.logcdf(0, loc=-μ_star_l, scale=jnp.sqrt(σ2_star_l))+norm.logpdf(u, loc=μ_l,scale= jnp.sqrt(σ2+σ2_l))


def alpha_beta(k,V,ρ, ν):
    """ Obtain vectors α and β for a given index k.
        α and β are used to calculate the conditional expectation of Zk - see derivations
    """
    Vk = jnp.asarray(V)[k]
    ρ_ν_k=jnp.asarray(ρ-ν)[k,:]
    ρ_ν_diag=jnp.diag(ρ-ν)
    α_k = Vk - ρ_ν_k/ρ_ν_diag * V
    β_k = ρ_ν_k/ρ_ν_diag
    α_k = α_k.at[k].set(0)
    β_k = β_k.at[k].set(1)
...
def form_numerator_k(k, V, u, ϕ_j, ρ, ν, σ, μ_l, σ2_l, μ_star_l, σ2_star_l):
    """ Obtain the numerator of the expectation E(Z_k|V,u,ϕ_j) for a given index k"""

    α_k, β_k = alpha_beta(k,V,ρ, ν)
    L = μ_l.shape[0]
    σ_l = jnp.sqrt(σ2_l + 1e-10) # adding this for numerical stability
    σ_star_l = jnp.sqrt(σ2_star_l + 1e-10)
    σ2 = jnp.power(σ, 2)

    assert α_k.shape[0]==L
    assert β_k.shape[0]==L
    assert (β_k*μ_l).shape[0]==L
    assert (σ2_l).shape[0] == L
    assert (β_k*μ_star_l).shape[0]==L
    assert (σ2_star_l).shape[0] == L

    pdf_0 = norm.pdf(0, loc=μ_l, scale=σ_l)
    pdf_cdf_ratio_0 = (pdf_0>1e-12).astype(int)*pdf_0/(norm.cdf(0, loc=μ_l, scale=σ_l)+1e-30)

    pdf_1 = norm.pdf(0, loc=μ_star_l, scale=σ_star_l)
    pdf_cdf_ratio_1 = (pdf_1>1e-12).astype(int)*pdf_1/(1-norm.cdf(0, loc=μ_star_l, scale=σ_star_l)+1e-30)

    terms_0 = (α_k+β_k*μ_l)-β_k*σ_l*pdf_cdf_ratio_0
    terms_1 = (α_k+β_k*μ_star_l)+β_k*σ_star_l*pdf_cdf_ratio_1
    terms = jnp.concatenate((terms_0, terms_1))
    log_terms = jnp.log(jnp.abs(terms))
    signs = 2*(terms>=0)-1

    coeffs_0 = norm.cdf(0,loc=μ_l, scale=σ_l)*norm.pdf(u,loc=0,scale=σ)*ϕ_j
    coeffs_1 = (1-norm.cdf(0, loc=μ_star_l, scale=σ_star_l))* norm.pdf(u, loc=μ_l,scale= jnp.sqrt(σ2+σ2_l + 1e-10))*ϕ_j
    coeffs = jnp.concatenate((coeffs_0, coeffs_1))*signs

    res, res_sign = logsumexp(a=log_terms, b=coeffs, return_sign=True)

    return res,res_sign


form_numerator = jax.jit(jax.vmap(form_numerator_k, in_axes=(0,None,None,None,None,None,None,None,None,None,None), out_axes=(0,0)))


def E_Z(V, u, ϕ_j, σ, ρ, ν, κ_T):
    L = ρ.shape[0]
    ϕ_col = ϕ_j.reshape((L, 1)).flatten()

    Σ = ν.T @ jnp.linalg.inv(ρ) @ ν + κ_T     # covariance matrix of V
    Σ_Z_given_V = (ρ - ν.T @ jnp.linalg.inv(Σ) @ ν)
    μ_l = ν @ jnp.linalg.inv(Σ) @ (V.reshape(L,1).flatten())
    σ2_l = Σ_Z_given_V.diagonal().flatten()
    ...
    log_num_arr, sign_num_arr = form_numerator(jnp.arange(L), V, u, ϕ_col, ρ, ν, σ, μ_l, σ2_l, μ_star_l, σ2_star_l)
    log_theta_0_all = form_log_theta_0_all(u, σ2, μ_l, σ2_l)
    log_theta_1_all = form_log_theta_1_all(u, σ2, μ_l, σ2_l)

    # the denominator is the same for all Z 
    log_denom, sign_denom = logsumexp(a = jnp.concatenate((log_theta_0_all,log_theta_1_all)), b = jnp.concatenate((ϕ_col,ϕ_col)), return_sign=True)
    diff = (log_num_arr - log_denom)
    # overflow happened here -> bounding the output
    #diff = (diff<1e2)*diff + (diff>=1e2)*1e2
    res = jnp.exp(diff)

    return (sign_num_arr * sign_denom *res).reshape((1, L))
...
def g_Z_j(V, Z, c_j, ϕ_j, ϵ_j, ρ, ν, κ_T, σ):
    L = ρ.shape[0]
    # jax.debug.print("q().shape: {x}", x = q_j_sample(Z, c_j, σ, L).shape)
    # jax.debug.print("q(): {x}", x = q_j_sample(Z, c_j, σ, L))
    return g_j(V, q_j_sample(Z, c_j, ϵ_j, L)[0], ϕ_j, σ, ρ, ν, κ_T)
# TODO: Need to replace the above with fresh samples, and make sure we are always using fresh samples (similar to logistic)

def dgdZ_j(V_j, Z_j, c_j, ϕ_j, ϵ_j, ρ, ν, κ_T, σ):
    L = ρ.shape[0]
    return jax.jacfwd(g_Z_j, 1)(V_j, Z_j, c_j, ϕ_j, ϵ_j, ρ, ν, κ_T, σ).reshape((L, L)) 
dgdZ_j_jit = jax.jit(dgdZ_j, static_argnames=("σ")) # LxL
dgdZ = jax.jit(jax.vmap(dgdZ_j, in_axes=(0, 0, 0, 1, 0, None, None, None, None), out_axes=0), static_argnames=("σ")) # nxLxL

# Updated with map vs. vmap
def κ_B_marginal(ϕ, ρ, n, σ, ν, κ_T, ρ_est = None, num_samples = 1000):
    ...
    η = jnp.array(nprandom.normal(size=(n, num_samples)) * σ)
    ...
    def E(ϕ_and_j_col_j): 
        ...
        def sm(c_j):
            ...
            u = q_j_sample(Z_B[j, :, :], c_j, η[j, :], L) 
            # TODO: This has to be updated/corrected, with a proper q_j_sample just like in the logistic case.
...
# Updated with map vs. vmap. TODO: update with the corrected q_j_sample syntax
...
def GAMP_full(B̃, δ, p, ϕ_, σ, X, Y, C_true, T = 10, B̂_0 = None, 
    ...
    for t in tqdm(range(T), disable = tqdm_disable):
        ...
        def sm(ϕ_j, j, c_j):
            ...
            u = q_j_sample(Z_B_fixed[j, :, :], c_j, η[j, :], L)
            # g_j_ = g_j(V[j, :].flatten(), q_j(Z_B[j, :].reshape((1, L)), c_j, σ).sample(), ϕ_j, σ, ρ, ν, κ_T).reshape((1, L))
            # NOTE: input V, u to g are sampled according to the true signal prior, but g itself
            # is defined using the estimated signal prior ρ_est.
            g_j_full = jax.vmap(g_j, in_axes=(0, 0, None, None, None, None, None), 
        ...
        def estimate_ν̂(V, Z, η_i):
            dgdZ_ = dgdZ(V, Z, C_true, ϕ, η_i, ρ_est, ν, κ_T, σ)
        ...
        # if type(est_signal_prior) == GaussianSignal:
            ...
            # this method of estimating v relies on sampling B according to the distribution determined by the state evolution, it does so based on the values of other state evolution params
...
def SE_fixed_C_v1(C_true, true_signal_prior: SignalPrior, \
    ...
    for t in (range(T)):
        ...
        def estimate_ν̂(V, Z, ϵ):
            ...
            # g designed assuming random C so ν should be ν_avg, κ_T should be κ_T_avg.
            # ϕ is just the ground truth (we're not assuming mismatch here).
            dgdZ_AD = dgdZ(V, Z, C_true, ϕ, ϵ, ρ_est, ν_avg, κ_T_avg, σ) # nxLxL, axis 1 = output (g) dim, axis 2 = input (V) dim,    
            ν̂_fixed_est = jnp.mean(dgdZ_AD, axis=0) # sum over n gives LxL matrix,
            assert ν̂_fixed_est.shape == (L, L)
            return ν̂_fixed_est
        ε = jax.random.normal(subkeys[2], shape=(n, num_samples)) * σ
