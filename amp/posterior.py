...
from amp.signal_configuration import η_to_ψ_jax, η_to_ψ_jax_combinations_mapped
...
# TODO: Exact posterior will now have to average over the possible signal configurations for a given η. Needs some work to adapt this. 
# In order to do this, will need to create another taublate_SE function that tabulates for all possible signal config combinations. 
...
def tabulate_state_evolution(η_arr, true_signal_prior, est_signal_prior, \
    ...
    def SE_fixed_C_clean(ψ):
        return amp.marginal_separable_jax.SE_fixed_C_v1(ψ, true_signal_prior, est_signal_prior, \
                                    δ, p, ϕ_, L, σ, T, \
...
### ---- Functions for the (approximate) posterior --- ###
...
logP_Vj_Yj_given_ψj_allj = jax.jit(jax.vmap(logP_Vj_Yj_given_ψj, (0, 0, 0, 
    None, None, None, None), 0)) # length-n vector
# If Vmap is too memory intensive here, use map. 
logP_Vj_Yj_given_ψj_mapped = jax.jit(jax.vmap(logP_Vj_Yj_given_ψj, (0, 0, 0, 
    None, None, 0, 0), 0)) # length-n vector
